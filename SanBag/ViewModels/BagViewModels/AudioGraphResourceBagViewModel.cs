...
{
    public class AudioGraphResourceBagViewModel : GenericBagViewModel
    {
        public AudioGraphResourceBagViewModel(BagViewModel parentViewModel)
                : base(parentViewModel)
        {
            ExportFilter += "|JSON File|*.json";
            CurrentResourceView = new AudioGraphResourceView();
            CurrentResourceView.DataContext = new AudioGraphResourceViewModel();
        }

        public override bool IsValidRecord(FileRecord record)
        {
            return record.Info?.Resource == FileRecordInfo.ResourceType.AudioGraphResource &&
                   record.Info?.Payload == FileRecordInfo.PayloadType.Payload;
        }

        protected override void CustomFileExport(ExportParameters exportParameters)
        {
            var Material = AudioGraphResource.Create();
        ...
        {
            var view = CurrentResourceView.DataContext as AudioGraphResourceViewModel;
            if (view == null)
            {
                return;
            }
